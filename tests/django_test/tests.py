...
from currency.fields import NotSupportedLookup
from currency.money import Money
from l10n.models import Currencies
import logging
log = logging.getLogger('currency_test')
class MoneyFieldTestCase(TestCase):
    ...
    def testCreating(self):
        ind = 0
        cur = Currencies.objects.currency_list()
        for code, currency in cur:
            ind = ind + 1
            price = Money(ind*1000.0, code)
            log.debug("price: %i %s" % (ind*1000.0, code))
            Entity.objects.create(name=currency.name, price=price)
        count = Entity.objects.all().count()
        self.assertEqual(len(cur), count)
